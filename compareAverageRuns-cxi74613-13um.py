...
files = ["output_runs-gdvn-13um-all_T+Q_20ADUs.h5"]
tags = ["20ADUs-all"]
...
class img_class (object):
	...
	def draw_img_for_viewing_water(self):
		...
		pngtag = original_dir + "peakfit-gdvn-13um_%s.png" % (self.filename)
		P.savefig(pngtag)
		print "%s saved." % (pngtag)
		epstag = original_dir + "peakfit-gdvn-13um_%s.eps" % (self.filename)
		P.savefig(epstag, format='eps')
		print "%s saved." % (epstag)
		P.close()
		#P.show()
...
if options.peakfit:
	...
	for i in range(len(files)):
		csvtag = "peakfit-gdvn-13um_%s.csv"%(tags[i])
	...
	P.savefig(original_dir + "peakfit-gdvn-13um_vs_T.png")
	print "peakfit-gdvn-13um_vs_T.png saved."
	P.savefig(original_dir + "peakfit-gdvn-13um_vs_T.eps", format='eps')
	print "peakfit-gdvn-13um_vs_T.eps saved."
	#P.show()
	P.close()
	...
	P.savefig(original_dir + "peakfit-gdvn-13um_vs_dist.png")
	print "peakfit-gdvn-13um_vs_dist.png saved."
	P.savefig(original_dir + "peakfit-gdvn-13um_vs_dist.eps", format='eps')
	print "peakfit-gdvn-13um_vs_dist.eps saved."
	#P.show()
	P.close()
	
	
	#for i in range(len(distances)):
		#txttag = "peakfit-gdvn-13um_%.1fmm.txt"%(distances[i])
		...
		#csvtag = "peakfit-gdvn-13um_%.1fmm.csv"%(distances[i])
